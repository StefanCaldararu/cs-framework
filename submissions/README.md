...
A file named `source_file.<extension>`, along with a `Makefile` must be submitted for each submission. Additionally, bash build scripts and other files (such as [cmake](https://github.com/StefanCaldararu/cs-framework/blob/main/submissions/c_submission/CMakeLists.txt)) can be included in the submission folder if necessary. The `Makefile` must be structured as follows:
...
### Cleaning
In this example, all installed `pip` dependancies are removed. This is not necessary, but good practice to provide a clean environment for other submissions.
> [!NOTE]
...
## C++ Submission
### Building
The c++ submission is built similarly to the C submission, but additionally includes an external dependancy ([eigen](https://eigen.tuxfamily.org/index.php?title=Main_Page)). This external dependancy is built from a provided bash script.

### Running
The executable generated by the build script is run.

### Cleaning
For this example, both all cloned and built directories are removed when cleaning.
